...
def extrair_bronze():
    ...
    # 1. Descompactar arquivos ZIP de 'data/landing' (recursivo) e 'data_dump' (incluindo aninhados)
    processed_zips = set()
    zips_to_process = list((data_path / "landing").rglob("*.zip")) + list(dump_path.glob("*.zip"))
    ...
    for source in [data_path / "landing", dump_path, temp_extract_path]:
